...
if {$argc == 0 || [string equal -nocase [lindex $argv 0] "-h"]} {
	send_user "\nUsage: scriptname \[-h\] \[-L <library-file>\] \[-p <password-file>\] \[-iplist <ip1 ip2 ...>\] \[-ipfile <file name>\]\n"
	send_user "\t\t\[-dcs <device config script>\] \[-dvs <device verification script>\] \[-drs <device rollback script>\]\n"
	exit 0
}
